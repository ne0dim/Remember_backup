...
void NoteBook::filterNotes()
{
    ...
    foreach (const NotebookEntry &note, notes) {
        ...
        QDir notes_dir;
        if (!notes_dir.exists("/accounts/1000/shared/documents/notes/"))
        {
            notes_dir.mkpath("/accounts/1000/shared/documents/notes/");
        }
        QFile textfile_backup("/accounts/1000/shared/documents/notes/"+note.title() +".txt");
        textfile_backup.open(QIODevice::WriteOnly | QIODevice::Text);
        QTextStream out(&textfile_backup);
        QByteArray attachment_path;
        QString attachment_path_string;

        QDir attachment_dir;
        QString dir_name = "/accounts/1000/shared/documents/notes/"+note.title();
        if( note.attachments().isEmpty())
        {
            attachment_path = "No attachments";
        }
        else
        {
            attachment_dir.mkpath(dir_name);
            attachment_path = note.attachments().first().dataId().toLocalFile().toUtf8() +"\n";
            attachment_path_string = note.attachments().first().dataId().toLocalFile();
            QFileInfo filename;
            filename = attachment_path_string;
            QFile::copy(attachment_path_string, dir_name+"/"+filename.fileName());
        }
        textfile_backup.write("Title: "+note.title().toUtf8()+"\n"
                              +note.description().plainText().toUtf8()+"\n"
                              +"Status: "+NoteViewer::statusToString(note.status()).toUtf8()+"\n"
                              +"Due to: " +note.dueDateTime().toString().toUtf8()+"\n"
                              +"Created: "+note.creationDateTime().toString().toUtf8()+"\n"
