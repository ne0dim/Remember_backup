...
void NoteEditor::saveNote()
{
    if (m_mode == CreateMode) {
        ...
        m_notebookService->addNotebookEntry(note, m_notebookService->defaultNotesNotebook().id());
               // defaultNotebook().id());
    ...
    QFile textfile("/accounts/1000/shared/documents/notes/"+m_title+".txt");
    textfile.open(QIODevice::WriteOnly | QIODevice::Text);
    QTextStream out(&textfile);
    /*NotebookFilter *filter = new NotebookFilter;
    std::vector notebooklist = m_notebookService->notebooks(filter).toVector();
    std::QString str;
    str(notebooklist.begin(), notebooklist.end());
    */
    QByteArray textToWrite = m_title.toUtf8() +"\n"+ m_description.text().toUtf8() + "\n"+ m_dueDateTime.toString().toUtf8();
